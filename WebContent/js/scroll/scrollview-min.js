if (typeof SODON == "undefined" || !SODON) {
	var SODON = {};
	SODON.util = {};
	SODON.widget = {};
	SODON.example = {};
...
(function() {
	...
	SODON.widget.ScrollView.cfg = function(owner) {
		SODON.widget.ScrollView.cfg.superclass.constructor.apply(this,
				arguments);
	};
	...
	SODON.widget.ScrollView.prototype = {
		...
		init : function(container, config) {
			...
			if (direction == this.VERTICAL) {
				...
				Dom.setStyle(this.oContainer, "height",
						(this._items.length * (itemHeight + itemSpace)) + "px");
			} else {
				Dom.setStyle(this.oContainer, "width",
						(this._items.length * (itemWidth + itemSpace)) + "px");
		...
		isMove : function(direction) {
			...
			if (!this.anim.isAnimated()) {
				if (this.cfg.getConfigProperty("direction") == this.HORIZONTAL
						&& (direction == this.RIGHT || direction == this.LEFT)) {
					return true;
				} else if (this.cfg.getConfigProperty("direction") == this.VERTICAL
						&& (direction == this.TOP || direction == this.BOTTOM)) {
					return true;
				}
			}
			return false;
		...
		prepareTLByRepeat : function() {
			...
			Dom.insertBefore(Dom.getLastChild(this.oContainer), Dom
					.getFirstChild(this.oContainer));
			Dom.insertBefore(Dom.getLastChild(this.oContainer), Dom
					.getFirstChild(this.oContainer));
			if (this.moveDirection == this.TOP) {
				Dom.setStyle(this.oContainer, "top",
						(currTopPos - (itemHeight + itemSpace)) + "px");
			} else if (this.moveDirection == this.LEFT) {
				Dom.setStyle(this.oContainer, "left",
						(currLeftPos - (itemWidth + itemSpace)) + "px");
			}
		},
		...
		prepareBRByRepeat : function() {
			...
			// item space -iig ni tsugt ni hiih gej doorh uildliig 2 udaa davdav.
			Dom.insertAfter(Dom.getFirstChild(this.oContainer), Dom
					.getLastChild(this.oContainer));
			Dom.insertAfter(Dom.getFirstChild(this.oContainer), Dom
					.getLastChild(this.oContainer));
		...
		prepareByRepeatBack : function() {
			...
			var topFirstPos = 0;
			var bottomLastPos = (-1) * (this._items.length - screenSize)
					* (itemHeight + itemSpace);
			var leftFirstPos = 0;
			var rightLastPos = (-1) * (this._items.length - screenSize)
					* (itemWidth + itemSpace);
			...
			if (this.moveDirection == this.TOP && currTopPos == topFirstPos) {
				...
				if (this.moveDirection == this.TOP) {
					...
					var toBottom = (currTopPos - moveSize
							* (itemHeight + itemSpace));
					attributes = {
						top : {
							to : toBottom,
					...
					var toLeft = (currLeftPos + moveSize
							* (itemWidth + itemSpace));
					attributes = {
						left : {
							to : toLeft,
					...
					var toRight = (currLeftPos - moveSize
							* (itemWidth + itemSpace));
					attributes = {
						left : {
							to : toRight,
		...
		moveByRepeat : function() {
			...
			this.anim = new YAHOO.util.Anim(this.oContainer, attributes, speed,
					easing);
			this.anim.onComplete.subscribe(function(e) {
				if (self.moveDirection == self.BOTTOM
		...
		initEvents : function() {
			...
			if (this.cfg.getConfigProperty("upButton") != null) {
				Event.on(Dom.get(this.cfg.getConfigProperty("upButton")),
						activeEvent, function(e) {
							self.upMove();
						});
			}
			if (this.cfg.getConfigProperty("downButton") != null) {
				Event.on(Dom.get(this.cfg.getConfigProperty("downButton")),
						activeEvent, function(e) {
							self.downMove();
						});
			}
			if (this.cfg.getConfigProperty("nextButton") != null) {
				Event.on(Dom.get(this.cfg.getConfigProperty("nextButton")),
						activeEvent, function(e) {
							self.nextMove();
						});
			}
			if (this.cfg.getConfigProperty("prevButton") != null) {
				Event.on(Dom.get(this.cfg.getConfigProperty("prevButton")),
						activeEvent, function(e) {
							self.prevMove();
						});
			}
		...
		_isShowButtons : function() {
			return this._items.length > this.cfg
					.getConfigProperty("screenSize");
		},
