...
function Dialog(content, options) {
	...
	var isIe6 = !$.support.opacity && !$.support.style
			&& window.XMLHttpRequest == undefined;
	/* 对话框的布局及标题内容。*/
			/*
	var barHtml = !options.showTitle
			? ''
			: '<div class="nwsl1"><span class="title">' + options.title
					+ '</span><a class="close">' + options.closeText
					+ '</a></div>';*/
	//   var dialog = $('<div id="' + options.id + '" class="dialog">'+barHtml+'<div class="content"></div></div>').hide();
	var titleHtml='<span class="title">' + options.title
					+ '</span>';
   var barHtml = !options.showTitle
			? ''
			: '<div class="bar"><a class="close">' + options.closeText
					+ '</a></div>';
	var dialog = $('<div id="' + options.id
			+ '" class="dialog"><div class="title">'+titleHtml+'</div><div class="content"></div>' + barHtml
			+ '</div>').hide();
	$('body').append(dialog);
	...
	this.show = function() {
		...
		var getOpacity = function(id) {
			...
			return (undefined != el && undefined != el.filters
					&& undefined != el.filters.alpha && undefined != el.filters.alpha.opacity)
